                     ...
                     Real a_new)
{
    ...
    if (std::abs(time-prev_time) > (1.e-10*cur_time) )
    {
        ...
        {
            std::cout << "just_the_mhd:  prev_time = " << prev_time << std::endl;
            std::cout << "just_the_mhd:       time = " <<      time << std::endl;
        }
        amrex::Abort("time should equal prev_time in just_the_mhd!");
    }
    ...
    if (S_old.contains_nan(Density, S_old.nComp(), 0))
    {
        ...
        {
            if (ParallelDescriptor::IOProcessor())
                std::cout << "just_the_mhd: testing component " << i << " for NaNs" << std::endl;
